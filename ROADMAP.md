...
- [x] **Transport Layer:** Implemented an agnostic, backend-swappable network layer behind the `NetworkManager` trait (`redixel-core::net`). The native backend (`redixel-net`) uses **WebTransport (QUIC/HTTP-3)** via `wtransport`, unifying reliable and unreliable delivery over a single encrypted connection instead of a separate UDP + WebRTC stack — `ReliableOrdered` (framed streams, capped at 4 MiB per frame) and `UnreliableSequenced` (datagrams with RFC 1982 sequence framing, newest-wins, fragmented and reassembled when a message exceeds the datagram limit — never promoted onto the reliable stream, so ordering holds and large snapshots cannot head-of-line block reliable traffic). The handshake validates a `protocol_id` and enforces `max_clients`. Ships with both self-signed (LAN) and PEM (production domain) TLS certificate modes, plus a `LoopbackNetwork` for tests and single-process hosting.
- [x] **Headless Mode:** `RuntimeConfig::headless()` + `HeadlessRuntime` run the engine with no `winit`/`wgpu`, driving only `on_start` and the fixed-update loop — the same `Game` implementation runs unmodified as a Linux VPS dedicated server.
- [x] **Fixed Update Loop:** `on_fixed_update` runs on a deterministic accumulator (`TimeManager`/`SimulationCore`) shared by the windowed and headless runtimes, with a spiral-of-death clamp (`max_substeps`) isolating simulation from render framerate.
- [x] **Network API:** `ctx.network()` on `GameContext` exposes `poll()` for `Connected`/`Disconnected`/`Message` events, `send`/`broadcast` over `ReliableOrdered`/`UnreliableSequenced`, plus `rtt()`, `is_connected()`, and authoritative tickrate negotiation (`server_tickrate()`) so a connecting client adopts the server's simulation rate automatically.
- [ ] **State Serialization:** Still open at the engine level — `NetworkManager` intentionally moves opaque `&[u8]` payloads with no built-in (de)serialization or compression helpers; games are expected to bring their own (e.g. `serde` + a compact codec) entirely in user code. A reusable engine-level utility (and the `SequenceBuffer` prediction/reconciliation primitive already shipped in `redixel-core::net` suggests it) is still a candidate for a future story.
- [ ] **Browser (WASM) Transport:** Not yet implemented — `redixel-net`'s `wasm` module logs an error and degrades to `NoOpNetwork`, so a WASM build of a networked game still runs (offline) instead of panicking. Bringing WebTransport (or a browser-compatible fallback) to WASM clients is intentionally deferred to a separate future story rather than bundled into this pass.
