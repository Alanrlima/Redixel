...
- [ ] **Browser (WASM) Transport:** Not yet implemented — `redixel-net`'s `wasm` module and `redixel::run_wasm_with` are stubs (`todo!()`). Bringing WebTransport (or a browser-compatible fallback) to WASM clients is intentionally deferred to a separate future story rather than bundled into this pass.
